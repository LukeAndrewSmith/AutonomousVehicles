...
float Car::accelerate(Road* road, int car_ahead_id) {
    ...
    if ( human && ( starting_lane == 1 && ( ( diff<safety_distance && diff>diff_end_event ) || ( diff_end_event < safety_distance && !(road->get_car_merging_id(car_ahead_id) ||  road->get_car_merged_id(car_ahead_id)) ) ) ) ) diff = diff_end_event;
        ...
        if ( !human ) {                                                                     // Autonomous Vehicles Protocol:
            ...
            if ( starting_lane == 0 ) {                                                 // top lane == lane to merge from
                ...
                if ( !road->get_car_merging_id(car_to_match_id) ) {
                    ...
                    float diff_end_event = (road->get_end_event()-safety_distance)-x_pos;
                    if ( ( diff<safety_distance && diff>diff_end_event ) || ( diff_end_event < safety_distance && !(road->get_car_merging_id(car_to_match_id) ||  road->get_car_merged_id(car_to_match_id)) ) ) {
                        tmp_accel_param = accelerate(road, car_to_match_id);
                    } else if ( wait != 0 ) {
                        wait--;
                        ...
                        if ( diff_merging < safety_distance/2 ) {
...
//    if ( id == road->get_n_cars_per_lane() ) {
////        std::cout << tmp_accel_param << ", " << velocity << std::endl;
//        if ( tmp_accel_param == min_accel_param ) {
//            std::cout << tmp_accel_param << std::endl;
//        }
//    }
