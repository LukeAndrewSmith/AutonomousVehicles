...
public:
    Car(int id, float init_velocity, float init_max_velocity, float init_x_pos, float init_y_pos, float init_accel_param, float init_max_accel_param, float init_min_accel_param, float init_car_length, float init_horizon, float init_max_delta_turning_angle);
    int update_decision(Road* road, float time_step);
...
public:
    ...
    int update_car_decisions(float time_step);
...
struct init_experiment {
    ...
    // Road
    int init_road_length;
    int init_speed_limit;
    int init_n_lanes;
    float begin_event;                  // ( begin_event < 0 ) => no event
...
public:
    ...
    void main_loop(std::string experiment_num, bool multi_lane, bool event); // event => wether an event (braking/lane merge is happening)
